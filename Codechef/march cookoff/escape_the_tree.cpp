...
First, Chef will lock his friends in the root of a full binary tree with depth h, i.e. with 2h+1−1 nodes (numbered 1 through 2h+1−1). The root has number 1, but the other nodes may be numbered arbitrarily. His friends can only escape this tree by reaching a special leaf node. They do not have any other information about the structure of the tree or the special node.
...
Choose a node x. Chef's response to this query is the distance between node x and the special node.
Choose a node x and an integer y. Consider all leaf nodes at the distance y from node x. If there is at least one such node, Chef chooses one such node and his response is the number of the node he chose. Otherwise, Chef's response is 0.
...
To ask a query of the first type, you should print a line containing two space-separated integers 1 and x (1≤x≤2h+1−1). Then, you must read a line containing a single integer ― the answer to your query, or −1 if the query is invalid or if you have asked too many queries.
To ask a query of the second type, you should print a line containing three space-separated integers 2, x and y (1≤x≤2h+1−1, 0≤y≤2h). Then, you must read a line containing a single integer ― the answer to your query, or −1 if the query is invalid or if you have asked too many queries.
...
3 3
                            1
Note
Example case 1: This tree is used by Chef. The special leaf node is node 3.
*/